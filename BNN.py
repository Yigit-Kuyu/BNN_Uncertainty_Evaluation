...
np.savez('data.npz', x_obs=x_obs, y_obs=y_obs, x_true=x_true, y_true=y_true)
xlims = [-0.5, 1.5]
ylims = [-1.5, 2.5]
...
def run_mcmc(x_obs, y_obs, num_samples=50, seed=42):  # Define the function with input data, number of samples, and seed
    ...
    # Run MCMC
    first_run=1 # 0 if we want to load the samples from the file for speed up the process
    if first_run==1:
...
def run_vi(x_obs, y_obs, num_samples=50, seed=42):  # Define the function for variational inference
    ...
    elbo_losses = []  # List to store ELBO losses

    # Run variational inference 
    for step in range(num_samples):
        loss = svi.step(x_train, y_train)  # Execute the variational inference step
        elbo_losses.append(loss)  # Store the ELBO loss
        if step % 10 == 0:
    ...
    return samples_vi, model, elbo_losses  # Return ELBO losses as well
